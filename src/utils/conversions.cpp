...
/////////////////////////////
btTransform toBulletTransform(const tf::StampedTransform& in) {
	btTransform out;
...
////////////////////////////////
tf::Transform toTFTransform(const btTransform& in) {
	tf::Transform out;
	btVector3 origin = in.getOrigin();
	btQuaternion rotation = in.getRotation();

	out.setOrigin(tf::Vector3(origin.x(), origin.y(), origin.z()));
	out.setRotation(tf::Quaternion(rotation.x(), rotation.y(),rotation.z(),rotation.w() ));
	return out;
}
